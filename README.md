...
Семантическая похожесть не хранится как ручная связь. Агент использует её как вычисляемый сигнал для инсайтов и предложений.
...
- `confirm` усиливает оба узла и сохраняет metadata для последующего выбора ручного типа связи; edge без явного `used_in`, `derived_from` или `contradicts` не создаётся.
- `refine` усиливает оба узла, сохраняет `--note` как уточнение и metadata для последующего выбора ручного типа связи; edge без явного типа не создаётся.
...
| Кнопка | API action | Эффект |
| --- | --- | --- |
| `Confirm` | `confirm` | Усиливает оба узла и сохраняет pending metadata для выбора ручного типа связи. |
| `Reject` | `reject` | Помечает узлы как `connection_rejected`, снижает их `interest_score`. |
| `Refine` | `refine` | Усиливает оба узла, сохраняет заметку и pending metadata для выбора ручного типа связи. |
...
| `POST` | `/api/edges` | Создать ручную связь `used_in`, `derived_from` или `contradicts`. Новые связи получают `origin=user`, `trust_status=confirmed`, `review_status=accepted`. |
