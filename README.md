...
- Проактивный агент:
  ...
  - LLM-проверка потенциальных противоречий;
...
Сохраняет текст как один узел `excerpt` без LLM-извлечения. Это CLI-аналог выделения текста в web-читалке.
...
Команда создаёт один `KnowledgeFragment`, один `Node` типа `excerpt`, не создаёт связей и не обращается к LLM.
...
MVP-читалка работает с локальными UTF-8 текстовыми, Markdown и HTML-файлами как с plain text. Файл читается в браузере через `FileReader` и не загружается на сервер целиком. На сервер отправляется только выбранный фрагмент.
...
- metadata файла и смещения выделения `offset_start`, `offset_end`, `selected_length`.

Сохранённый фрагмент сразу участвует в старом анализе: агент может найти для него напоминание, скрытую связь или противоречие.
...
| `GET` | `/reader` | Простая web-читалка для ручного сохранения выделений. |
...
| `POST` | `/api/manual-fragments` | Сохранить выделенный текст как `excerpt` без LLM-обработки. |
...
2. `find_hidden_connections` сравнивает пары узлов по лексическому сходству и локальным embeddings.
3. `find_contradictions` отправляет похожие пары в LLM для проверки противоречия.
4. `generate_digest` применяет персонализацию, сортирует инсайты и обрезает список до `AGENT_DIGEST_LIMIT`.
5. Дайджест сохраняется в `<STORAGE_PATH>.insights.json`, если запуск был с сохранением.
...
Тесты покрывают модели, репозиторий, LLM-извлечение, CLI, API, проактивного агента, импорт источников, web UI, feedback и персонализацию.
...
- CLI;
- REST API;
- ручное добавление `excerpt`-фрагментов без LLM;
- проактивный агент;
- дайджесты инсайтов;
...
Следующий этап по `PROGRESS.md`: тестирование, документация и подготовка к пользовательской проверке.
