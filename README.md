...
Семантическая похожесть не хранится как ручная связь. Агент использует её как вычисляемый сигнал для инсайтов и `AgentProposal`. Старые persisted типы вроде `related_to`, `supports`, `example_of`, `part_of` и `similar_to` не входят в новую MVP-модель; перед запуском новой версии старый storage лучше очистить через `python -m app.cli clear`.
...
- `confirm` усиливает оба узла и создаёт ручную связь `used_in` по умолчанию. Через API можно передать другой MVP-тип в `edge_type`.
- `refine` усиливает оба узла, сохраняет `--note` как уточнение и создаёт ручную связь `used_in` по умолчанию.
...
| Кнопка | API action | Эффект |
| --- | --- | --- |
| `Confirm` | `confirm` | Усиливает оба узла и создаёт manual `used_in`-связь по умолчанию. |
| `Reject` | `reject` | Помечает узлы как `connection_rejected`, снижает их `interest_score`. |
| `Refine` | `refine` | Усиливает оба узла, сохраняет заметку и создаёт manual `used_in`-связь по умолчанию. |
