...
- Граф знаний в памяти на NetworkX с canonical versioned JSON storage, atomic write/file lock и GEXF export/fallback.
- Хранение исходных фрагментов, предложений агента, дайджестов и пользовательских реакций в JSON.
- LLM-пайплайн извлечения сущностей и отношений из текста.
- Ручное создание и редактирование узлов и связей через API и `/graph`.
- Ручное сохранение выделенных фрагментов как `quote`-узлов без LLM-обработки, с выбором knowledge-типа и пользовательскими тегами.
- Структурированные provenance-привязки у узлов: `source_id`, URL/локальный путь, название, автор, даты, offsets, исходный фрагмент и комментарий.
...
- Web-читалка `/reader` для локальных UTF-8 текстовых и Markdown-файлов.
- Рабочий графовый интерфейс `/graph` для создания узлов, редактирования выбранного узла и ручного связывания двух выбранных узлов.
- Проактивный агент:
...
- `app/core/models.py` - модели `Node`, `Edge`, `KnowledgeFragment`, `AgentProposal`, перечисления типов узлов, связей и слоёв.
- `app/core/repository.py` - графовый репозиторий, CRUD, поиск, статистика, proposals, versioned storage, сохранение и загрузка.
...
- `metadata` - источник, provenance, исходное имя сущности, confidence, тема, feedback-метаданные.
- `provenance` - структурированная привязка к месту происхождения знания: `source_id`, `source_url`, `document_title`, `author`, `published_at`, `added_at`, `source_type`, `position`, `offset_start`, `offset_end`, `source_text`, `user_comment`.
- `trust_status` - `confirmed`, `suggested`, `auto_inferred`, `conflict` или `needs_clarification`.
- `origin` - `user`, `llm`, `agent` или `system`.
- `review_status` - `pending`, `accepted`, `rejected` или `edited`.
- `user_comment`, `title`, `tags` - пользовательский комментарий, короткий заголовок и теги.
- `strength` - сила памяти от `0` до `1`.
- `decay_rate` - скорость забывания в день.
- `last_interacted` - последнее взаимодействие.
- `created_at` - дата создания.
- `user_id`, `graph_id` - граница данных; по умолчанию `local` и `default`.
...
`source`-узел создаётся только явным ручным действием через API/граф. `/reader` и ручной capture не превращают файл, URL или цитату в смысловой `source`-узел: источник хранится как provenance у созданного knowledge-узла.
...
- `edge_layer` - слой связи: `manual`, `service` или `suggested`.
- `metadata` - описание, источник, feedback-метаданные.
- `trust_status`, `origin`, `review_status`, `user_comment` - статус доверия, происхождение, статус проверки и комментарий.
- `created_at` - дата создания.
...
Семантическая похожесть не хранится как ручная связь. Агент использует её как вычисляемый сигнал для инсайтов и `AgentProposal`. Старые persisted `used_in`-связи при загрузке нормализуются в `related_to`.
...
- `llm_status`, `warnings`, `errors` - честный статус LLM-извлечения.
- `created_at` - дата добавления.

При ручном добавлении мысли с источником `Node.content` хранит мысль пользователя, `Node.source_text` хранит цитату/абзац-основание, а `KnowledgeFragment.content` сохраняет источник как provenance-журнал добавления. Для одного и того же URL или локального файла capture переиспользует стабильный `source_id`, поэтому в `/graph` можно подсветить узлы с общим источником без создания графовой связи. `derived_from` остаётся только смысловой ручной связью между knowledge-узлами.
...
Важно: в проекте нет алгоритмического fallback-извлечения. Если LLM-клиент не настроен или запрос к модели завершился ошибкой, фрагмент будет сохранён, но `nodes_created` и `edges_created` будут равны `0`, а API вернёт `llm_status`, `warnings` и `errors`.
...
Для `--file` по умолчанию используется `source_type=file`, `source_url=<path>`. Для `--url` по умолчанию используется `source_type=url`, `source_url=<url>`. URL-импорт разрешает только `http`/`https`, запрещает localhost/private IP по умолчанию, проверяет text-like `content-type` и ограничивает размер ответа. HTML пока сохраняется как полученный текст без отдельного парсинга статьи.

Выводит ID созданного фрагмента или список фрагментов, количество созданных узлов и общее число узлов.

### `add-manual`

Сохраняет текст как один узел без LLM-извлечения. Без `--source-text` команда создаёт `quote`; с `--source-text` создаёт `idea`, где `content` - мысль, а `source_text` - исходная цитата или абзац.
...
- `confirm` усиливает оба узла и создаёт ручную связь `related_to` по умолчанию. Через API можно передать другой MVP-тип в `edge_type`.
- `refine` усиливает оба узла, сохраняет `--note` как уточнение и создаёт ручную связь `related_to` по умолчанию.
...
Удаляет сохранённые файлы текущего хранилища. Перед запуском версии с MVP 2-моделью выполните эту команду, если в storage лежит старый граф с legacy-типами:
...
| Кнопка | API action | Эффект |
| --- | --- | --- |
| `Confirm` | `confirm` | Усиливает оба узла и создаёт manual `related_to`-связь по умолчанию. |
| `Reject` | `reject` | Помечает узлы как `connection_rejected`, снижает их `interest_score`. |
| `Refine` | `refine` | Усиливает оба узла, сохраняет заметку и создаёт manual `related_to`-связь по умолчанию. |
...
| `Add as Idea` | Кладёт выделенный текст в редактируемое поле `Idea` в правой панели. |
...
| `Add Idea` | Сохраняет текст из поля `Idea` как `idea`; поле `Source` отправляется как `source_text`. |
...
- для быстрого сохранения выделения: metadata файла и смещения `offset_start`, `offset_end`, `selected_length`; сервер присваивает общий `source_id` для этого файла/URL;
- для мысли с источником: `text=<мысль>`, `source_text=<источник>`, `node_type=idea`, а также metadata смещения источника `source_offset_start`, `source_offset_end`, `source_selected_length`, если источник был взят из выделения.

Сохранённый узел сразу участвует в следующем анализе. Reader не создаёт `source`-узлы и не создаёт `derived_from`-связь к источнику; provenance остаётся UI/metadata-привязкой.

### Graph

Графовый интерфейс доступен на:

```text
http://127.0.0.1:8000/graph
```

В боковой панели выбранного узла показываются title/URL источника, `source_text`, offsets и другие provenance-поля. Если у узла есть `source_url`, доступна ссылка `Open Source`. Кнопка `Highlight Same` подсвечивает другие узлы с тем же `source_id`; это не создаёт ребро в графе.
...
| `POST` | `/api/knowledge` | Добавить текст, извлечь знания через LLM, сохранить фрагмент, узлы и связи; ответ содержит `llm_status`, `warnings`, `errors`. |
| `POST` | `/api/sources` | Импортировать прямой текст или текстовый URL. |
| `POST` | `/api/manual-fragments` | Сохранить выделенный текст как `quote` или мысль с `source_text` как `idea` без LLM-обработки; принимает явный `node_type` и `tags`. |
| `GET` | `/api/nodes` | Получить список узлов. Поддерживает `node_type`, `search`, `limit`. |
| `POST` | `/api/nodes` | Создать ручной узел MVP 2-типа. Новые узлы получают `origin=user`, `trust_status=confirmed`, `review_status=accepted`. |
...
| `POST` | `/api/edges` | Создать ручную связь `related_to`, `supports`, `contradicts`, `derived_from`, `example_of` или `clarifies`. Новые связи получают `edge_layer=manual`, `origin=user`, `trust_status=confirmed`, `review_status=accepted`. |
...
curl -X POST http://127.0.0.1:8000/api/manual-fragments ^
  -H "Content-Type: application/json" ^
  -d "{\"text\":\"Моя мысль по прочитанному.\",\"source_text\":\"Абзац книги, на котором основана мысль.\",\"node_type\":\"idea\",\"source_type\":\"reader\",\"document_title\":\"book.md\"}"
```

Пример обновления provenance у существующего узла:

```bash
curl -X PATCH http://127.0.0.1:8000/api/nodes/<node_id>/provenance ^
  -H "Content-Type: application/json" ^
  -d "{\"source_url\":\"https://example.com/article\",\"document_title\":\"Article\",\"offset_start\":120,\"offset_end\":188,\"source_text\":\"Quoted source text\"}"
```
...
6. Инсайты также сохраняются как reviewable proposals в `<STORAGE_PATH>.proposals.json`: hidden connection -> `proposed_edge`, contradiction -> `possible_contradiction`, reminder -> `reminder`.
7. Дайджест сохраняется в `<STORAGE_PATH>.insights.json`, если запуск был с сохранением.
8. Состояние анализа сохраняется в `<STORAGE_PATH>.analysis_state.json`; следующий запуск пропускает пары, где оба узла и параметры анализа не изменились.
...
| Файл | Содержимое |
| --- | --- |
| `data/graph.graph.json` | Canonical storage графа с `schema_version`, узлами, связями, фрагментами и proposals. |
| `data/graph.gexf` | Export/fallback формат графа для совместимости с NetworkX/GEXF tooling. |
...
- CLI;
- REST API;
- ручное добавление `quote`-фрагментов и `idea`-мыслей с `source_text` без LLM;
- структурированные provenance-привязки у узлов, обновление provenance через API и подсветка общего `source_id` в `/graph`;
- проактивный агент;
