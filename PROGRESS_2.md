...
**Текущий этап:** после Этапа 3.  
**Следующий рекомендуемый фокус:** Этап 2 - источники как provenance-привязки.
...
Текущий автоматический тестовый набор: **94 теста проходят**.
...
## Этап 2. Источники как provenance-привязки ⏳ (частично, примерно 25%)
...
- [ ] Нет стандартизированной модели source attachment / provenance record у узла.
- [ ] Нет API для создания/обновления provenance-привязки у узла.
- [ ] Reader пока не переиспользует общий `source_id` для узлов из одного файла/URL.
- [ ] В graph UI пока нет фильтра/подсветки кластера узлов с одним источником.
- [ ] Нет удобной кнопки открытия URL/локального источника из боковой панели graph UI.
- [ ] Нет тестов на создание, обновление, переиспользование и отображение source attachment.
...
- `source_text` сейчас является контекстом происхождения знания; source должен развиваться как provenance-привязка, а не как автоматически создаваемый смысловой `source`-узел.
