...
Exocortex - персональная система управления знаниями на основе графа знаний и LLM. MVP умеет хранить узлы и связи, прикреплять к узлам структурированный provenance источников, извлекать сущности из текста через настроенный LLM, вручную сохранять выделенные фрагменты без LLM-обработки, анализировать граф проактивным агентом, формировать дайджесты инсайтов, принимать реакции пользователя, обновлять граф по feedback, строить базовый профиль интересов, импортировать внешние текстовые источники и работать через CLI/FastAPI.
