...
Графовый интерфейс для ручного создания узлов и связей доступен на `/graph`.
...
- `POST /api/manual-fragments` - сохранить выделенный текст как `quote` или мысль с `source_text` как `idea` без LLM-обработки; можно передать явный `node_type` и `tags`
...
- `POST /api/nodes` - создать ручной узел MVP 2-типа с `origin=user`, `trust_status=confirmed`, `review_status=accepted`
- `GET /api/nodes/{node_id}` - узел по ID
- `PATCH /api/nodes/{node_id}` - отредактировать узел
...
Откройте `http://127.0.0.1:8000/reader`, загрузите UTF-8 текстовый или Markdown-файл. Выделенный фрагмент по умолчанию сохраняется как `quote`, но перед сохранением можно выбрать любой MVP 2-тип и указать теги. Пользовательская мысль в правой панели по умолчанию сохраняется как `idea`; для неё тоже можно изменить тип и добавить теги. Сценарий для изучения книги: напишите мысль, выделите абзац-источник и выберите `Add as Source`, при необходимости отредактируйте источник или скройте его кнопкой `Hide`, затем нажмите `Add Idea`. Читалка отправляет на сервер мысль, источник, имя файла, смещения выделения и стандартное поле `tags`; LLM при этом не вызывается.

Откройте `http://127.0.0.1:8000/graph`, чтобы создать узел напрямую в графе, отредактировать выбранный узел в правой панели или выбрать два узла как source/target и создать ручную связь `used_in`, `derived_from` или `contradicts` с пользовательским комментарием.
...
Реализованы Этап 1 и Этап 3 MVP 2; источники как полноценные graph-сущности, suggestion model и review queue остаются отдельными следующими этапами:
...
- Docker Compose
- 94 автоматических теста

Следующий рекомендуемый этап по `MVP_PLAN_2.md`: Этап 2, источники как полноценная часть графа.
