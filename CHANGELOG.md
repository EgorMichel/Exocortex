...
- **Модели данных** (`app/core/models.py`):
  - `Node`: Узел графа знаний с полями id, тип, содержание, редактируемый `source_text`, метаданные, параметры памяти (strength, decay_rate, last_interacted)
...
- **Ручной захват фрагментов** (`app/services/manual_capture.py`):
  ...
  - Сохранение ручной мысли как `thesis`: `content` хранит мысль, `source_text` хранит источник, `KnowledgeFragment.content` сохраняет источник как provenance
...
- **Тесты** (`tests/`):
  ...
  - Текущий набор: 89 автоматических тестов
